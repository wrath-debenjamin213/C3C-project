...
import { Users, User, CheckCircle, Circle, Play } from 'lucide-react';
...
const TeamCollaboration = ({ roadmap, onUpdate }) => {
    const teamMembers = useMemo(() => {
        const size = parseInt(roadmap.teamSize, 10) || 1;
        ...
        for (let i = 0; i < size; i++) {
            ...
            members.push({
                id: `u${i + 1}`,
                name: size > PREDEFINED_MEMBERS.length ? `Member ${i + 1}` : memberTemplate.name,
        ...
        <div className="fade-in" style={{ maxWidth: '900px', margin: '0 auto', paddingBottom: '2rem' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(200px, 1fr))', gap: '1rem', marginBottom: '3rem' }}>
                {memberStats.map(member => {
                    const progress = member.totalTasks === 0 ? 0 : Math.round((member.completedTasks / member.totalTasks) * 100);
                        ...
                        <div key={member.id} className="glass-panel" style={{ padding: '1.5rem', borderTop: `4px solid ${member.color}`, background: member.bg }}>
                            ...
                            <div style={{ marginBottom: '1rem' }}>
                                ...
                                <div style={{ width: '100%', height: '6px', background: 'rgba(255,255,255,0.1)', borderRadius: '3px', overflow: 'hidden' }}>
                                    <div style={{ height: '100%', width: `${progress}%`, background: member.color, transition: 'width 0.3s ease' }}></div>
                                </div>
                            </div>

                            <div className="flex justify-between" style={{ fontSize: '0.85rem', color: 'var(--text-secondary)', borderTop: '1px solid rgba(255,255,255,0.05)', paddingTop: '0.5rem' }}>
                ...
                {roadmap.milestones.map(m => (
                    <div key={m.id} className="glass-panel" style={{ padding: '1.5rem', borderLeft: '4px solid var(--panel-border)' }}>
                        ...
                        <div className="flex-col gap-2">
                            {m.tasks.map(t => {
                                const currentAssignee = t.assigneeId || 'unassigned';
